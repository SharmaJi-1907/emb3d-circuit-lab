...
CircuitLab (`emb3d-circuit-lab`): a browser electronics lab with a 3D component viewer, circuit simulator, component database, board pinouts, datasheets and an assistant. Vanilla JS, Vite 5, Three.js r128 from CDN, Canvas 2D. No framework.
...
- Known leftovers are **warnings**, capped by `--max-warnings` in `package.json` (currently 9, issue E11). When a fix removes warnings, lower the cap to the new count. Never raise it.
...
- A test fails on any uncaught exception or `console.error`. Known noise is listed in `IGNORED_CONSOLE_ERRORS` in [tests/smoke/helpers.js](tests/smoke/helpers.js), tagged with its issue code. Remove an entry when that issue is fixed. Only one is left: the Google Fonts CDN (E14), because a failure there is not a bug in this app.
...
- Files: `app.spec.js` (screens, data, 3D first load, search, AI), `keyboard.spec.js` (shortcuts, explode), `styles.spec.js` (computed styles of shared building blocks), `viewer.spec.js` (3D Viewer layout and sizing), `database.spec.js` (Component Library in the Database screen), `ai.spec.js` (AI chat: sending, layout, message styles, which answer is picked, safe text, code blocks, screen styles), `simulator.spec.js` (Simulator layout, controls, multimeter, circuit logic, oscilloscope, drawing loop), `projects.spec.js` (Projects grid, new-project form, saving and deleting), `theme.spec.js` (light/dark theme: both toggles, persistence, readability), `topbar.spec.js` (notifications drawer, search arrow keys, New Project and Share buttons), `assets.spec.js` (what the page loads: manifest, favicon, no unused CDNs, no dead CSS), `viewer-models.spec.js` (3D model pin counts, chip labels, freeing old models, passive parts), `background.spec.js` (page background: one animation, theme colours, dots after a resize), `routing.spec.js` (hash routing: address, links, refresh, Back/Forward), `boards.spec.js` (Board Explorer layout, styles, behaviour and board data), `datasheet.spec.js` (Datasheet sidebar and section-bar styles, list clicks, layout fit).
- **3D tests are slower than the rest**: headless Chrome draws 3D on the CPU (about 15 fps with 1 browser, about 5 fps with 4). `viewer.spec.js` runs 9 tests in about 25 s (~2.8 s each) while `projects.spec.js` runs 7 in about 8 s (~1.2 s each); the full suite is about 2.6 min. Use `openViewer()` from `helpers.js` in 3D tests. `openApp()` does **not** wait for the 3D engine (E15) — call `waitForViewer()` if your test needs it. Engine animations are **time-based**; keep new ones time-based too.
