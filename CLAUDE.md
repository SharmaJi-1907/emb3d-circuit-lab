...
- Files: `app.spec.js` (screens, data, 3D first load, search, AI), `keyboard.spec.js` (shortcuts, explode), `styles.spec.js` (computed styles of shared building blocks), `viewer.spec.js` (3D Viewer layout and sizing).
- **The suite is slow while the Viewer is the start screen** (about 1.7 min): headless Chrome draws 3D on the CPU (about 15 fps with 1 browser, about 5 fps with 4). The 3D scene isn't drawn while hidden, so this drops once the Dashboard is home (#8). Engine animations are **time-based**; keep new ones time-based too.
