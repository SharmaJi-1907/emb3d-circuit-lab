...
| Datasheet Viewer | ✅ Shows datasheets. Sidebar, section bar and layout styled (#17); the screen no longer cuts off its bottom. The search box is still dead (C8) and the "Pinout" section has no data (D26) |
...
| C8 | 🟠 | Datasheet | The **search box** (`ds-search`) does nothing. No JS anywhere references it: `grep -n "ds-search" src/app/app.js` returns no matches, so typing in it never filters the list. _Found in #17 while styling the sidebar._ |
...
| D26 | 🟡 | **The Datasheet "Pinout" section has no data**, so clicking it shows the placeholder "Section content under catalog index division." Meanwhile a `package` section **does** have data but has no button. The buttons are Overview, Pinout, Electrical, Timing, Memory, Examples; the data sections are overview, electrical, examples, **package**, timing, memory. _Measured in #17: 5 of the 6 buttons show content, "Pinout" shows the placeholder._ Fix by writing a pinout section for both datasheets, or by swapping the button to "Package". | [index.html:470](../index.html#L470), [data.js:811](../src/data/data.js#L811) |
