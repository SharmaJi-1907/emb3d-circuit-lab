...
| E15 | 🟡 | **Every smoke test waits for the 3D engine, even screens with no 3D.** `openApp()` in `helpers.js` waits for `ThreeViewer.isReady()`, so all tests block on software-rendered WebGL starting up, not just the Viewer ones. CLAUDE.md says "only Viewer tests pay this cost", which is not true today. _Found in #18: the suite grew to 516 slots under `--repeat-each=4` and 4 tests timed out at 30 s with no assertion failing — 3 of them on the Projects screen, which shows no 3D. Raising the timeout to 60 s unblocked it, but the waiting is the real cost._ Fix by waiting only for `CircuitApp` in `openApp()` and letting `openViewer()` wait for the engine, then lower the timeout again and correct the CLAUDE.md note. | [helpers.js:60-63](../tests/smoke/helpers.js#L60-L63), [playwright.config.js:8](../playwright.config.js#L8) |
